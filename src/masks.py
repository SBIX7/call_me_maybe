...
class MaskManager:
    ...
    def apply_mask(self, logits: List[float], allowed_wrds: List[str]) -> None:
        """Mask logits to allow tokens starting with exact allowed words."""
        valid_tokens_id = set()
    ...
    def apply_number_mask(self, logits: List[float], len_param: int) -> None:
        """Specialized mask to allow numeric chars and JSON symbols."""
    ...
    def apply_string_mask(self, logits: List[float]) -> None:
        """Mask to allow free text but strictly prevent newlines."""
        forbiden_tokens = {"\n", "\r", "\t", "Ċ", "č", "ĉ"}
        valid_token_id = set()
